...
const c = document.getElementById("konstanta");
const m = document.getElementById("modulus");
const form = document.getElementById("form");
const totalNumber = document.getElementById("total");

function multiplicative(z0, a, c, m, randomNumbers, totalNumber) {
	randomNumbers[0] = z0;

	for(let i = 1; i < totalNumber; i++) {
		let temp = ((randomNumbers[i-1] * a) + Number(c)) % m;
...
form.addEventListener('submit', function(e) {
	...
	multiplicative(z0.value, a.value, c.value, m.value, randomNumbers, totalNumber.value+1);
	getRandomNumbers(randomNumbers, m.value, resultNumbers, totalNumber.value+1);

	asumsi.innerHTML += `a = ${a.value}, c = ${c.value}, m = ${m.value}, z0 = ${z0.value}`;
	...
	for(let i = 0; i < totalNumber.value; i++) {
		...
		<tr>
			<td>${i+1}</td>
			<td>${randomNumbers[i]}</td>
			<td>Z${i+1} = (${a.value} * ${randomNumbers[i]} + ${c.value}) mod ${m.value} = ${randomNumbers[i+1]}</td>
